...
console.log("🚀 Ajout du filtre Terra Classic pour les News...");
...
// On utilise les versions françaises de CoinTelegraph et du Journal du Coin
const FEEDS = [
...
export async function GET() {
  try {
    const allNews = [];
    // Mots-clés obligatoires pour garder l'article
    ...
    const feedPromises = FEEDS.map(async (feed) => {
      try {
        ...
        parsed.items.slice(0, 30).forEach((item) => {
          ...
          // On vérifie si l'article parle de Terra Classic
